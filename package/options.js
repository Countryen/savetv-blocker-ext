...
var GLOBAL_SELECTED_ENTRY_LI__old = null;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById("clear__old").addEventListener("click", clear__old);
  document.getElementById("import__old").addEventListener("click", import__old);
  document.getElementById("export__old").addEventListener("click", export__old);
...
// ========================== __OLD__
function clear__old() {
  var setNumber = GLOBAL_SELECTED_ENTRY_LI__old.__OLD__NUMBER;
  storage.setNewArray("old_entries_" + setNumber);
  document.getElementById("notify").innerHTML = "";
  document.getElementById("notify").textContent = new Date().toLocaleString() + ": " + "Set mit Nummer=" + setNumber + " wurde geleert";
  document.getElementById("entry-actions__old").innerHTML = "";
  update();
  GLOBAL_SELECTED_ENTRY_LI__old = null;
}

function export__old() {
  var setNumber = GLOBAL_SELECTED_ENTRY_LI__old.__OLD__NUMBER;
  storage.getOneAsync("old_entries_" + setNumber, function(items) {
    var saveJsonObject = { };

    var entries = items["old_entries_" + setNumber];
    ...
    document.getElementById("entry-actions__old").innerHTML = "";
    document.getElementById("entry-actions__old").appendChild(a);  
  });
}

function import__old() {
  var setNumber = GLOBAL_SELECTED_ENTRY_LI__old.__OLD__NUMBER;
  ...
  loadInput.id = "load-input";
   ...
   loadButton.onclick = function() {
    ...
    fileReader.onload = fileReader_onload__old; // see below.
    fileReader.readAsText(selectedFile);

    function fileReader_onload__old(fileReaderEvent) {
      ...
      var o = {};
      o["old_entries_" + setNumber] = entries;
      storage.set(o);
      document.getElementById("entry-actions__old").textContent = "";
      document.getElementById("notify").textContent = new Date().toLocaleString() + ": " + "Set mit Nummer=" + setNumber + " wurde importiert";
      update();
      GLOBAL_SELECTED_ENTRY_LI__old = null;
    }
  }
  document.getElementById("entry-actions__old").innerHTML = "";
  document.getElementById("entry-actions__old").appendChild(loadInput);
  document.getElementById("entry-actions__old").appendChild(loadButton);
}
...
function update() {
    storage.getOneAsync("entries", function(items) {
      ...
      if (!GLOBAL_SELECTED_ENTRY_LI__old) {
        document.getElementById("manage-entry__old").style.display = "none";
      } else {
        document.getElementById("manage-entry__old").style.display = "block";
        document.getElementById("manage-entry-title__old").textContent = GLOBAL_SELECTED_ENTRY_LI__old.textContent;
      }
    ...
    storage.getSpaceOfOneAsync("entries", function(spaceInBytes) {
    ...
    //__OLD__
    storage.getSpaceOfOneAsync("old_entries_1", function(spaceInBytes) {
      var spaceStruct = storage.convertBytesToBiggestUnit(spaceInBytes);
      var li__old_1 = document.createElement("li");
      li__old_1.textContent = "#1: " + spaceStruct.amount.toFixed(2) + "(" + spaceStruct.unit + ")";
      li__old_1.__OLD__NUMBER = "1";
      li__old_1.addEventListener("click", function() { GLOBAL_SELECTED_ENTRY_LI__old = this; update(); });
      while (document.getElementById('list__old').firstChild) {
        document.getElementById('list__old').removeChild(document.getElementById('list__old').firstChild);
      }
      document.getElementById('list__old').appendChild(li__old_1);
    });
